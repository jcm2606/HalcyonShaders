...
*/
  ...
  vec3 drawReflections(in GbufferData gbufferData, in PositionData positionData, in vec3 diffuse, in vec2 screenCoord, in mat2x3 atmosphereLighting, in vec4 highlightTint, in vec2 dither) {
    ...
    vec3 specular = raytraceRough(vec3(screenCoord, positionData.depthFront), positionData.viewFront, normal, viewDirection, gbufferData.roughness, vec3(gbufferData.f0), _pow(gbufferData.skyLight, 6.0), dither);
