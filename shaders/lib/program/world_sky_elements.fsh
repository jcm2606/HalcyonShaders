...
// Main.
void main() {
    vec4 albedo = texture2D(texture, uvCoord) * tint;
    float alphaSign = sign(albedo.a);

    vec3 normal = vec3(0.5, 0.5, 1.0);
         normal = tbn * (normal * 0.5 + 0.5);

    /* DRAWBUFFERS:01 */
    gl_FragData[0] = vec4(EncodeAlbedo(albedo.rgb), 0.0, Encode4x8F(vec4(lmCoord, 0.0, 0.0)), 1.0);
