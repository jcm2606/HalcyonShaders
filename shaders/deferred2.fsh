...
void main() {
  ...
  // DRAW REFLECTIONS
  buffers.tex0.rgb = (getLandMask(position.depthBack)) ? drawReflectionOnSurface(buffers.tex0, screenCoord, position.depthBack, position.viewBack, gbuffer.albedo, gbuffer.normal, gbuffer.roughness, gbuffer.f0, gbuffer.skyLight, getAtmosphereLighting(), buffers.tex5) : buffers.tex0.rgb;
